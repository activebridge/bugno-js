...
function buildGruntKarmaConfig(singleRun, browsers, tests, reporters) {
  ...
  for (var testName in tests) {
    ...
    if (testName === 'requirejs') {
      testConfig.files = [
        {src: './dist/bugno.umd.js', included: false}
      ];
...
module.exports = function(grunt) {
  ...
  var bugnoJsSnippet = fs.readFileSync('dist/bugno.snippet.js');
  var bugnojQuerySnippet = fs.readFileSync('dist/plugins/jquery.min.js');
    ...
    replace: {
      snippets: {
        ...
        replacements: [
          // Main bugno snippet
          {
            from: new RegExp('^(.*// Bugno Snippet)[\n\r]+(.*[\n\r])*(.*// End Bugno Snippet)', 'm'),
            to: function(match, index, fullText, captures) {
              captures[1] = bugnoJsSnippet;
              return captures.join('\n');
            }
          },
          // jQuery bugno plugin snippet
          {
            from: new RegExp('^(.*// Bugno jQuery Snippet)[\n\r]+(.*[\n\r])*(.*// End Bugno jQuery Snippet)', 'm'),
            to: function(match, index, fullText, captures) {
              captures[1] = bugnojQuerySnippet;
              return captures.join('\n');
            }
          },
          // README travis link
          {
            from: new RegExp('(https://api\.travis-ci\.org/bugno/bugno\.js\.png\\?branch=v)([0-9a-zA-Z.-]+)'),
            to: function(match, index, fullText, captures) {
  ...
  grunt.registerTask('copyrelease', function createRelease() {
    ...
    builds.forEach(function (buildName) {
      var js = 'dist/bugno' + buildName + '.js';
      var minJs = 'dist/bugno' + buildName + '.min.js';

      var releaseJs = 'release/bugno' + buildName + '-' + version + '.js';
      var releaseMinJs = 'release/bugno' + buildName + '-' + version + '.min.js';
